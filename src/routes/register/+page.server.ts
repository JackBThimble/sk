		...
		try {
			...
			if (!auth.validateUsername(validatedData.username)) {
				return fail(400, {
					...
					errors: {
						username:
							'Username must be alphanumeric and between 3 and 31 characters (may contain underscores and hyphens)'
					}
				});
			}
			...
			console.error('An error occurred during the registration.', e);
			return fail(500, {
				message: 'An error has occurred during registration',
				data: { username: userData.username },
				errors: {
					errors: 'An unknown error occured during the registration'
				}
			});
		}
