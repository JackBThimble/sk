...
export class GameStorage {
	...
	async saveScoreToServer(score: number, gameData?: any): Promise<boolean> {
		try {
			const { saveScore } = await import('$lib/services/gameApi');

			const result = await saveScore(this.gameId, score, gameData);
	...
	async loadHighScoresFromServer(limit = 10): Promise<any[]> {
		try {
			const { getHighScores } = await import('$lib/services/gameApi');

			const result = await getHighScores(this.gameId, limit);
