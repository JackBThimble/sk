...
import { AudioManager, type AudioAsset } from '../core/audio';
import { GameStorage } from '../core/storage';
import type { GameSettings, Vector2D, IGame } from '../core/types';
import { Direction, Difficulty, GameState } from '../core/types';
...
export class SnakeGame extends GameEngine implements IGame {
	...
	private setupInputHandlers(): void {
		// keyboard handlers
		const setupKeyHandler = (keys: string[], handler: (key: string) => void) => {
			keys.forEach((key) => {
				this.input.onKeyPress(key, handler);
			});
		};
		setupKeyHandler(['ArrowUp', 'w', 'W', 'k', 'K'], (key) => {
			if (this.direction !== Direction.DOWN) {
				this.nextDirection = Direction.UP;
			}
		});

		setupKeyHandler(['ArrowDown', 'j', 'J', 'S', 's'], (key) => {
			if (this.direction !== Direction.UP) {
				this.nextDirection = Direction.DOWN;
			}
		});

		setupKeyHandler(['ArrowRight', 'l', 'L', 'd', 'D'], (key) => {
		...
		// touch handlers
		this.input.onTouchStart((touches: TouchList) => {
